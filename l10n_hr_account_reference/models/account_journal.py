...
class AccountJournal(models.Model):
    ...
    def _l10n_constrain_pnb_model(self):
        for record in self:
            if (
                record.invoice_reference_type == 'hr' and
                record.invoice_reference_model not in REF_CROATIA
            ) or (
                record.invoice_reference_type != 'hr' and
                record.invoice_reference_model in REF_CROATIA
            ):
                raise ValidationError(_("Both Communication standard and Communication type,"
    ...
    invoice_reference_model = fields.Selection(
        ...
        ondelete={
            "00": "set default", "01": "set default",
            "02": "set default", "03": "set default",
            "06": "set default", "99": "set default"
        }
        )
    ...
    property_l10n_hr_P1_pnbr = fields.Selection(
        ...
        string='P1', help='1. ref number field.',
    )
    property_l10n_hr_P2_pnbr = fields.Selection(
        selection=_P1_P4_selection,
        company_dependent=True,
        string='P2', help='2. ref number field.',
    )
    property_l10n_hr_P3_pnbr = fields.Selection(
        selection=_P1_P4_selection,
        company_dependent=True,
        string='P3', help='3. ref number field.',
    )
    property_l10n_hr_P4_pnbr = fields.Selection(
        selection=_P1_P4_selection,
        company_dependent=True,
        string='P4', help='4. ref number field.',
    )
    ...
    def onchange_invoice_reference_model(self):
        if (
            self.invoice_reference_model in REF_CROATIA and
